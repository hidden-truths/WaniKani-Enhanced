...
The `db/client.ts` God-Object split shipped (commit `c78cc63`: 1,396-line module → `db/connection.ts` + `db/repos/*` behind a barrel, per-repo tests, 237 tests green). **Workstream D (schemas split) has now shipped too**; **two workstreams remain**, in recommended order — full steps/files/tests/dead-ends in the doc:
