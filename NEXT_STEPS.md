...
**Last updated**: 2026-06-14 — **独り言 Self-Talk slot-swap TEMPLATES → sentence store, Slice 1 (structure in DB) is merged to `main` + pushed** (commit `be2ee94`). A `sentence_template` table (own `public_template` view + a `getSentences`-mirroring privacy gate + a pinned breach test) holds the generator structure, served by `GET /v1/templates`; the client fetches it instead of importing the JS bundle; `seed-sentences.ts` grew a Pass 3. Curator-only (no authoring write path). **Slice 2 — lazy materialization of realizations as `sentence` rows + tooling — is the next slice** (kickoff in [TEMPLATES_KICKOFF_PROMPT.md](TEMPLATES_KICKOFF_PROMPT.md); design/phasing/open-Qs in [SENTENCE_STORE_TEMPLATES.md](SENTENCE_STORE_TEMPLATES.md)). NOT prod-deployed yet (no `seed-sentences.ts` Pass-3 run on the droplet — see §1/"Deploy"). Prior: **sentence-store Phase 4 (NLP enrichment) is built** on branch `sentence-store-phase4` (commits 1–3: offline GiNZA parser, N5/N4 grammar catalog, then `?annotate=1` serving + the study-app tap-a-word lookup + Browse grammar filter). Tests/build green + browser-verified; **pending merge + prod deploy** (incl. the new `seed-annotations.ts` step — §1 / "Deploy"). The flagged next rework is **tokenization granularity** (する-verbs + conjugations fragment the tap units — §1). Prior: the **study app's audio-unify epic is fully shipped AND deployed to prod**. This session: the unified sentence store Phase 2 (built-in examples), native audio for Minna cards in Browse/Reviews, explicit-voice authority + TTS ETag/revalidate fixes, and the prod Siri-voice rollout (clips pushed to Spaces via `push-tts-variants.ts` + manifest seeded on the droplet via `seed-audio-variants.ts`) all landed. Prod verified serving Phase 2 card sentences + the new `no-cache`/ETag TTS headers + the `siri:male`/`siri:female` voice picker. **The site is private — not published or announced anywhere** — so there is no public-launch work on the runway; the next stages are the sentence-store NLP phase + content/app polish.
...
### 1. Sentence-store NLP — Phase 4 SHIPPED (on a branch); the ⭐ next rework is tokenization granularity

**Phase 4 (NLP enrichment) is built** on branch `sentence-store-phase4` (commits 1–3: offline GiNZA parser + `sentence_annotation` load path, N5/N4 grammar-tag catalog, then commit 3 = `?annotate=1` serving + the study-app **tap-a-word lookup** (lemma/POS → card-or-Jisho) + the Browse **grammar filter**). Tests + build green; browser-verified. **Not yet merged or deployed** — see "Deploy" below: it needs `sentence-store-phase4` → `main`, a both-container rebuild, and **`seed-annotations.ts` run on prod after `seed-sentences.ts`** (skip it and the tap UI silently shows plain ruby in prod). Plans: **[SENTENCE_STORE_PHASE4.md](SENTENCE_STORE_PHASE4.md)** (as-built + the §8.0 rework) + [SENTENCE_STORE_NLP.md](SENTENCE_STORE_NLP.md).
...
- **Other sentence-store phases.** Beyond NLP (item 1): **Self-Talk TEMPLATES Slice 2** = lazily materialize slot-swap realizations as `sentence` rows (Slice 1 — structure in DB — is merged; this is the next slice; kickoff in [TEMPLATES_KICKOFF_PROMPT.md](TEMPLATES_KICKOFF_PROMPT.md), design in [SENTENCE_STORE_TEMPLATES.md](SENTENCE_STORE_TEMPLATES.md)); **Phase 2.5** = custom-card `ex` → private store rows (currently still rendered from the `custom-verbs` blob); **Phase 3** = Minna sentences → store (`public=0`). All deferred, all behavior-preserving. See [SENTENCE_STORE_NLP.md](SENTENCE_STORE_NLP.md) "Remaining phases".
