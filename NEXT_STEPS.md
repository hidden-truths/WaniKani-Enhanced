...
- **C — `record-compare.js` decomposition** (HIGH risk, optional): **C0 (the characterization-test net) ✅ SHIPPED** — the remaining inline pure helpers extracted to `core/recordings.js` + new `core/refs.js` (reference-variant selection + URL shapes + `parseControlCtx`, all `base`/`httpServed`/`prefs`-injected), with +39 tests; `record-compare.js` delegates via thin same-named wrappers (net −25 lines; 13 exports + both consumers unchanged); 180 tests + build green. **C1+** (the stateful-glue split into capture/takes/playback/waveform/view) **remains optional — under reassessment** (its pure logic is now fully in `core/`; the doc's decision point says stopping after C0 is legitimate). **Executable guide: [docs/RECORD_COMPARE_DECOMP.md](docs/RECORD_COMPARE_DECOMP.md).** Plus the **post-B enhancement follow-ups** (per-blob 409 merge; idempotency keys for sessions/recording uploads) — see [REFACTOR_FOLLOWUPS.md](REFACTOR_FOLLOWUPS.md) "Enhancement follow-ups".
