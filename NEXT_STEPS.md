...
- **C — `record-compare.js` decomposition** (~1–2 days, HIGH risk, optional): split the 853-line DOM/audio glue into capture/takes/playback/waveform/view (its pure logic is already in `core/`). Characterization-test net first; browser-verify each commit. **Executable guide: [docs/RECORD_COMPARE_DECOMP.md](docs/RECORD_COMPARE_DECOMP.md).** Plus the **post-B enhancement follow-ups** (per-blob 409 merge; idempotency keys for sessions/recording uploads) — see [REFACTOR_FOLLOWUPS.md](REFACTOR_FOLLOWUPS.md) "Enhancement follow-ups".
