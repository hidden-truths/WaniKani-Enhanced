...
**Last updated**: 2026-06-15 — **the `db/client.ts` God-Object split shipped to `main`** (the 1,396-line server repo module → `db/connection.ts` + `db/repos/*` behind a re-export barrel, with per-repo tests; commit `c78cc63`), **Workstream D (the `schemas.ts` split)** — the 733-line / 66-schema module → `src/schemas/*` behind a re-export barrel (byte-identical, OpenAPI unchanged) — and **Workstream B (study-app sync resilience)** — resilient transport + durable offline write-queue + the `SyncedBlob` collapse of all five sync trios + 409 optimistic concurrency, browser-verified end-to-end — **also shipped**. **All four SOLID/quality follow-up workstreams have now shipped** — the `db/client.ts` split, D (schemas split), B (sync resilience), and **C (record-compare decomposition): C0 pure-helper test-net (`core/recordings.js` additions + new `core/refs.js`, +39 tests) + C1+ full glue split into `features/record-compare/{state,capture,takes,playback,waveform,view}.js` behind a thin `record-compare.js` re-export, the 13-export contract + both consumers held byte-for-byte, 182 tests + build green** (live mic flow pending a manual browser pass); all recorded in [REFACTOR_FOLLOWUPS.md](REFACTOR_FOLLOWUPS.md) — see runway §4. **The post-B enhancement follow-ups (E1–E3) have now shipped too** (E1 per-blob 409 merge; E2 idempotency keys for sessions + the recording upload; E3 the upload routed through the resilient transport — 4 commits; study-app 198 tests, server 246, typecheck + build green). Prior context: sentence-store work is the active thread and several pieces landed on `main`: **Self-Talk slot-swap TEMPLATES Slices 1 + 2** (generator structure in the `sentence_template` table served by `GET /v1/templates`; combos lazily materialized as `sentence` rows via `POST /v1/templates/{id}/realize`), **Phase 2.5** (custom-card examples → private store rows), **custom-card completeness** (UI-authored pitch `accent` + leveled `levels`), and **Self-Talk TTS pre-gen** (phrases + every template combo). **Sentence-store Phase 4 (NLP enrichment) is also merged** (offline GiNZA parser → `?annotate=1` serving + the study-app tap-a-word lookup + Browse grammar filter). **The ⭐ next rework is tokenization granularity** — GiNZA morphemes fragment する-verbs + conjugations, so a tapped span doesn't match a word (§1). **Deploy residual:** the new seed steps (`seed-sentences.ts` Pass 3 + `seed-annotations.ts`) haven't run on the droplet, so prod doesn't yet serve templates/annotations (§1 / "Deploy"). The site is private — no public-launch work on the runway.
...
The `db/client.ts` God-Object split shipped (commit `c78cc63`: 1,396-line module → `db/connection.ts` + `db/repos/*` behind a barrel, per-repo tests, 237 tests green). **Workstreams D (schemas split) and B (study-app sync resilience) have now shipped too**; **only Workstream C remains** — full steps/files/tests/dead-ends in the doc:

- ~~**D — `schemas.ts` → per-domain modules**~~ **✅ SHIPPED** — the 733-line / 66-schema file → `src/schemas/{common,vocab,warm,accounts,progress,minna,audio,sentences,templates}.ts` behind a re-export barrel (byte-identical move; all 11 route consumers unchanged; typecheck + 237 tests green; `/openapi.json` identical). `templates` got its own module (mirrors `db/repos/templates.ts`).
- ~~**B — study-app sync resilience**~~ **✅ SHIPPED** (4 commits B1–B4): `net/transport.js` (timeout + idempotency-aware retry/backoff), `net/sync-queue.js` (durable offline write-queue), `features/synced-blob.js` (`SyncedBlob` collapsing all five trios incl. minna + queue/flush wiring), and server 409 optimistic concurrency on `PUT /v1/progress` (`baseUpdatedAt` compare-and-set, backward-compatible). +33 study-app tests, +7 server tests; browser-verified the offline→queue→reconnect-flush cycle + the live 409.
- ~~**C — `record-compare.js` decomposition**~~ **✅ SHIPPED** (8 commits): **C0** extracted the remaining inline pure helpers to `core/recordings.js` + new `core/refs.js` (reference-variant selection + URL shapes + `parseControlCtx`, all `base`/`httpServed`/`prefs`-injected, +39 tests); **C1+** split the 854-line glue into `features/record-compare/{state,capture,takes,playback,waveform,view}.js` behind `index.js`, with `record-compare.js` a thin `export *` re-export so minna.js/selftalk.js stayed byte-for-byte unchanged (13-export contract held). 182 tests + build green; browser-smoked (the live mic-driven record→compare→waveform→bias flow still wants a manual pass — headless blocks getUserMedia). **Executable guide: [docs/RECORD_COMPARE_DECOMP.md](docs/RECORD_COMPARE_DECOMP.md).**
- ~~**Post-B enhancement follow-ups**~~ **✅ SHIPPED** (4 commits): **E1** per-blob 409 merge — `createSyncedBlob` gained an injected `merge(local,server)`; on a 409 it unions local+server (no data loss) + re-pushes (single-round guarded), with pure per-blob mergers in [study-app/src/core/merge.js](study-app/src/core/merge.js) (settings stays server-wins). **E2** idempotency keys for `POST /v1/sessions` + the recording upload — a new guarded `ensureColumn`/`migrate` in [db/connection.ts](wk-enhanced-api/src/db/connection.ts) (SQLite has no `ADD COLUMN IF NOT EXISTS`) + a partial unique index; repos dedup on the key; client `logSession` now retries + offline-queues, the upload sends `?idem=`. **E3** the upload routed through the resilient transport (new `rawBody` path, `retry:true`; the multi-MB blob is intentionally not offline-queued). study-app 198 tests · server 246 · typecheck + build green. See [REFACTOR_FOLLOWUPS.md](REFACTOR_FOLLOWUPS.md) "Enhancement follow-ups".
