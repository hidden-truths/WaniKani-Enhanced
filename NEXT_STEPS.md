...
### 4. SOLID / code-quality refactor follow-ups — ✅ ALL SHIPPED ([archived record](docs/history/REFACTOR_FOLLOWUPS.md))

The whole SOLID/quality campaign is done — the `db/client.ts` God-Object split (`c78cc63`: 1,396-line module → `db/connection.ts` + `db/repos/*` behind a barrel) plus workstreams **D** (schemas split), **B** (sync resilience), **C** (record-compare decomp), **S** (songs decomp), **T** (cross-feature dedup), **M** (minna decomp), **U** (server read-through media cache — `mediaCache` + `singleFlight`), and the **E1–E3** enhancements. The full per-workstream record (problem/design/steps/files/tests/dead-ends) is in the archived doc linked above; the originally-planned subset is summarized below:
...
- ~~**Post-B enhancement follow-ups**~~ **✅ SHIPPED** (4 commits): **E1** per-blob 409 merge — `createSyncedBlob` gained an injected `merge(local,server)`; on a 409 it unions local+server (no data loss) + re-pushes (single-round guarded), with pure per-blob mergers in [study-app/src/core/merge.js](study-app/src/core/merge.js) (settings stays server-wins). **E2** idempotency keys for `POST /v1/sessions` + the recording upload — a new guarded `ensureColumn`/`migrate` in [db/connection.ts](wk-enhanced-api/src/db/connection.ts) (SQLite has no `ADD COLUMN IF NOT EXISTS`) + a partial unique index; repos dedup on the key; client `logSession` now retries + offline-queues, the upload sends `?idem=`. **E3** the upload routed through the resilient transport (new `rawBody` path, `retry:true`; the multi-MB blob is intentionally not offline-queued). study-app 198 tests · server 246 · typecheck + build green. See [REFACTOR_FOLLOWUPS.md](docs/history/REFACTOR_FOLLOWUPS.md) "Enhancement follow-ups".
