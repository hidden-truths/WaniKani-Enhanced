...
- **Phase 1 client coexistence shipped** in userscript v1.0.0-rc1/rc2 (see [CLIENT_MIGRATION.md](CLIENT_MIGRATION.md)). The userscript still runs the original IK/DDG/Google paths by default; a `useApiServer` setting opts into the API path. This means the server is exercised by real reviews today (locally) even though no public deployment has been done.
