// ==UserScript==
// @name         WK Vocab Review — ImmersionKit Examples (Legacy Direct)
// @namespace    https://github.com/jbrelly/wk-ik-examples
// @version      1.1.1-legacy
// @description  Frozen direct-path snapshot — calls ImmersionKit / DuckDuckGo / Google TTS from the browser. Install only if the WKEnhanced API server is unreachable.
// @author       jbrelly
...
(function () {
    ...
    const SCRIPT_TITLE = 'WK Vocab Review — ImmersionKit (Legacy Direct)';
    const SCRIPT_VERSION = '1.1.1-legacy';
    ...
    const DEFAULTS = {
        ...
        jlptPreferred: 'any',
        // ---- API server ----
        // This is the LEGACY direct-path snapshot — the API server path is
        // permanently disabled regardless of these stored settings (see
        // serverPathEnabled() below, which is hardcoded to return false).
        // The fields are kept in DEFAULTS so the settings dialog can still
        // render without throwing; they have no runtime effect.
        useApiServer: false,
        apiServerUrl: '',
        prefetchCount: 0,
    };
...
`;
    ...
    function serverPathEnabled() {
        // Legacy snapshot: API server path is permanently disabled. This
        // userscript exists as a fallback for users whose API server is
        // unreachable, so we hardcode false here even if the user has an
        // old `useApiServer: true` preference still stored in wkof.
        return false;
    }
