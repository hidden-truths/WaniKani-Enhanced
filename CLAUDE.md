...
As of mid-2026 the API server **also** hosts a third surface: a **Japanese verb-trainer study app** served at the apex `https://wkenhanced.dev/`, with email/password accounts and per-user progress sync. It's a single self-contained HTML file at [wk-enhanced-api/web/index.html](wk-enhanced-api/web/index.html) (derived from the standalone offline original at [japanese-study/japanese-verbs.html](japanese-study/japanese-verbs.html)), backed by `/v1/auth/*` + `/v1/progress/*` on the same server. This is separate from the userscript flow — it's a standalone study tool that happens to share the droplet. See the "Accounts + study app" section in [wk-enhanced-api/CLAUDE.md](wk-enhanced-api/CLAUDE.md) and the apex-hostname setup in [wk-enhanced-api/deploy/README.md](wk-enhanced-api/deploy/README.md).
...
5. [CLIENT_MIGRATION.md](CLIENT_MIGRATION.md) — migration plan (all three phases shipped).
