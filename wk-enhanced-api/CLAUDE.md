...
- **Local TTS pre-generation (macOS) + Minna-audio prefetch (`scripts/`):** operator scripts feed the surfaces above, all run locally and seed our storage (point them at the prod `S3_*` env to seed prod):
  ...
  - `push-tts-variants.ts` — copies ALREADY-RENDERED tagged voice clips (`audio/<provider>/<gender>/<hash>.m4a`, default provider `siri`) from the local media dir straight to a target bucket **without re-running `say`** — the companion to `generate-tts.ts` for seeding/fixing prod without rendering twice. Since rendering a Siri voice needs a Mac with the right System Voice, but the bytes are identical everywhere, this just ships the local `.m4a`s you already made. Run on the Mac with `STORAGE_DRIVER=s3 S3_*` pointed at prod; `--dry-run` first, then `--force` to OVERWRITE (required to **re-voice** clips seeded with the wrong System Voice — without it, existing keys are skipped). It only touches BYTES by default: the `audio_variants` manifest rows (the Settings-picker catalog) already exist on prod from the original `generate-tts.ts --variant` run and the hashes are unchanged, and a Mac invocation can't reach the droplet's sqlite anyway — so manifest upserts are opt-in via `--seed-manifest` (only meaningful when `DATABASE_FILE` points at the target env's DB). Scoped to provider dirs + `.m4a` so it can never sweep up the IK voice-actor `.mp3` media that shares the `audio/` namespace.
