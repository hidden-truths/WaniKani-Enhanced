...
- **Sentence store** (Phase 1 of the unified-sentence rearchitecture; backs 独り言 Self-Talk so far). One canonical row per sentence that surfaces REFERENCE by id instead of embedding text — the foundation for cross-surface reuse / de-dup / later NLP. `examples.js` + Minna are later phases.
  - `sentence` — `{ext_id, hash, text, furigana(JSON [{t,r?}]), source, public, visibility, created_by, created_at}`. `text` = `plainText(jp)` byte-for-byte; `hash` = `ttsTextHash(text)` computed server-side (the audio-layer key). `public=1, visibility='public', created_by=NULL` = curator (seeded); `public=0, visibility='private', created_by=<user>` = user-authored. Partial unique index `(hash) WHERE public=1 AND visibility='public'` — only the export/anon slice is unique-by-hash (private rows may share a hash; no global UNIQUE). `ON DELETE CASCADE` from `users`.
  - `translation` / `sentence_tag` — `{sentence_id, lang, text, ordinal}` and `{sentence_id, kind('scene'|'grammar'|…), value}` child rows.
  - `sentence_link` — polymorphic ownership `{owner_type('card'|'grammar_point'|'conversation'|'lesson'|'selftalk'), owner_id?, tier?, role?, ordinal, clip_*}`; Self-Talk uses `owner_type='selftalk'`. Card/grammar/conversation owners get wired in later phases (schema supports them now).
  - `sentence_annotation` — created now, **populated in the later NLP phase** (GiNZA tokens/bunsetsu by char offset into `text`).
  ...
  - **Privacy choke-point:** every read goes through `db.getSentences({ownerType, viewer})` which ALWAYS ANDs `(public=1 OR created_by=:viewer)`, fail-closed (null viewer → public only). Pinned breach-prevention tests live in `db/client.test.ts` — keep them green. Writes (`createSentence`/`updateUserSentence`/`deleteUserSentence`) enforce ownership in SQL; `upsertPublicSentence` is the idempotent seed path. Served by `routes/sentences.ts`; `/v1/sentences` is in the `STUDY_ROUTE` credentialed-CORS allowlist.
...
- **Local TTS pre-generation (macOS) + Minna-audio prefetch (`scripts/`):** operator scripts feed the surfaces above, all run locally and seed our storage (point them at the prod `S3_*` env to seed prod):
  ...
  - `seed-sentences.ts` — seeds the built-in 独り言 Self-Talk phrases (from `../../study-app/src/data/selftalk.js`) into the **sentence store** as public rows (`db.upsertPublicSentence`, idempotent). The git-tracked bundle stays the curator authoring source; this is the seed→DB step. **Must run as a deploy step** (point `DATABASE_FILE` at the prod sqlite / run on the droplet, same pattern as `generate-tts.ts`) or the prod 独り言 tab renders empty.
