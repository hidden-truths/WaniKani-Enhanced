...
Seven SQLite tables (schema in [src/db/schema.sql](src/db/schema.sql)):
...
- `recording/<userId>/<lesson>/<sanitizedItemKey>/<uuid>.webm` — a user's voice take (Minna Phase 2). **Private** object (`acl:'private'`) — served only via the gated `/v1/minna/recordings/{id}` route, never its public URL. The authoritative key lives in the `minna_recordings` row.
...
| GET | `/v1/tts?text=` | — | TTS for the study app. Three-tier cache: in-process → storage (`tts/<sha256(text)>.{m4a,mp3}`) → Google. Prefers a locally pre-generated Apple-voice `.m4a`; persists the Google `.mp3` to storage on first hit. `Cache-Control: immutable`. 400 missing/over-200-char, 502 upstream. |
...
| GET | `/v1/minna/audio?src=` | Cookie | Native-audio MP3 proxied from vnjpclub + cached in storage (strict `/Audio/…mp3` SSRF guard). Same gate. |
| POST | `/v1/minna/recordings?lesson&itemKey&durationMs&keep` | Cookie | Save a voice take (raw `audio/webm`/`mp4` body, ≤2 MB). Stores a **private** object; prunes the item to `keep` (≤20). Returns the new take + the item's take list. Same gate. |
| GET | `/v1/minna/recordings?lesson=` | Cookie | List the user's takes for a lesson (newest first; metadata only). Same gate. |
| GET | `/v1/minna/recordings/{id}` | Cookie | Stream one of the **owner's** recordings (`private` cache). 404 for a non-owner/missing id. Same gate. |
| DELETE | `/v1/minna/recordings/{id}` | Cookie | Delete one of the owner's recordings + its storage object (idempotent). Same gate. |
...
- **TTS proxy (storage-backed + Apple-voice-preferred):** `GET /v1/tts?text=<jp>` serves a three-tier cache, cheapest first: in-process map → our **storage layer** (`tts/<sha256(text)>.{m4a,mp3}` via `ttsKey()`) → `googleTts` (`tl=ja`, 200-char cap). A live Google clip is **persisted to storage** (`.mp3`) on first hit so it survives restarts and is never fetched again; a **locally pre-generated Apple-voice `.m4a`** (see the pre-generation pipeline below) is **preferred** over Google so the nicer on-device voice wins for any text we've voiced. `Cache-Control: immutable`. The study app falls back to speechSynthesis only over `file://` or on failure.
- **Local TTS pre-generation (macOS) + Minna-audio prefetch (`scripts/`):** three operator scripts feed the surfaces above, all run locally and seed our storage (point them at the prod `S3_*` env to seed prod):
  - `jp-tts.swift` — a macOS Swift CLI (`AVSpeechSynthesizer` → AAC `.m4a` via `afconvert`) that renders Japanese text to audio with Apple's on-device voices (Kyoko). Compiled binary `scripts/jp-tts` is gitignored; build with `swiftc -O scripts/jp-tts.swift -o scripts/jp-tts`.
  - `generate-tts.ts` — enumerates the text the study app voices (every built-in + Minna card reading via `ttsText`, every leveled/lesson example sentence via `plainText`), renders one `jp-tts` batch, and uploads each clip to `ttsKey(text,'m4a')`. Idempotent (skips clips already in storage).
