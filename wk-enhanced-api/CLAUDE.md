...
- **Local TTS pre-generation (macOS) + Minna-audio prefetch (`scripts/`):** operator scripts feed the surfaces above, all run locally and seed our storage (point them at the prod `S3_*` env to seed prod):
  - `generate-tts.ts` — enumerates the text the study app voices (every built-in + Minna card reading via `ttsText`, every leveled/lesson example sentence via `plainText`), renders each clip, and uploads it to `ttsKey(text,'m4a')`. Idempotent (skips clips already in storage). Two renderers via `--engine`:
