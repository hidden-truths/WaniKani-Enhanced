...
bun test                  # bun:test runner; 376 tests across 38 files, <1s
...
│   ├── etag.ts               # strong-ETag builder + weak-prefix-tolerant If-None-Match compare (vocab + index_meta)
│   ├── realize.ts            # byte-for-byte port of the study-app's realizeTemplate/plainText/rubyToSegments
│   ├── rateLimit.ts          # per-IP fixed-window limiter for the auth endpoints
│   ├── minnaGate.ts          # currentUser + MINNA_OWNER_EMAILS allowlist gate for /v1/minna/*
...
│   ├── httpErrors.ts         # unauthorized()/notFound() JSON error-response helpers shared across routes
│   ├── singleFlight.ts       # generic keyed in-flight coalescing (one upstream call per key under a concurrent herd); backs mediaCache (per-key, joins waiters) AND the warm pipeline's per-word ddgWarms (drops duplicates)
│   └── sleep.ts
...
│   ├── sessions.ts           # POST /v1/sessions — durable study-session log
...
└── minna/lesson-<n>.json     # curated みんなの日本語 lessons (git-tracked, from scripts/scrape-minna.ts)
```
...
- `user_progress` — per-user, per-app JSON progress blob (PK `(user_id, app)`; `app` ∈ {`verbs`, `custom-verbs`, `settings`, `minna`, `selftalk`, `songs`, `wanikani`, `jlpt`} — the progress store, the custom-verb definitions, the Settings-page preferences, the みんなの日本語 dashboard's per-lesson notes, the 独り言 Self-Talk practice signal, the 歌/Songs per-song progress (starred/shadowed lines), the 鰐蟹 WaniKani tab's WK API token, and the 合格 JLPT tab's exam plan (level + exam date + daily-checklist record) each sync as a separate row; the WK DATASET itself never reaches this server — it lives in the study-app's IndexedDB, re-syncable from api.wanikani.com). The cloud-synced replacement for the study app's localStorage. The blob is opaque to the server. `ON DELETE CASCADE` from `users`.
...
- **Sentence store** (unified-sentence rearchitecture; backs 独り言 Self-Talk **and** built-in vocab example sentences). One canonical row per sentence that surfaces REFERENCE by id instead of embedding text — the foundation for cross-surface reuse / de-dup / later NLP. Phase 1 = Self-Talk; **Phase 2 = built-in vocab examples** (`source='example'`, public rows linked to cards). Minna + NLP are later phases.
  - `sentence` — `{ext_id, hash, text, furigana(JSON [{t,r?}]), source('selftalk'|'example'|…), public, visibility, created_by, created_at}`. `text` = `plainText(jp)` byte-for-byte; `hash` = `ttsTextHash(text)` computed server-side (the audio-layer key). `public=1, visibility='public', created_by=NULL` = curator (seeded); `public=0, visibility='private', created_by=<user>` = user-authored; **`source='minna'` rows are GATED curator content (public=0, visibility='public', created_by=NULL — copyright-gated textbook material, Phase 3)** so they're DARK to the `getSentences` gate (`public=1 OR created_by=:viewer` is never true for them) and excluded from `public_sentence` — only the email-gated `/v1/minna` route reads them (`getMinnaAnnotations`/`getMinnaSentenceByExtId`, scoped to `source='minna'`). Partial unique index `(hash) WHERE public=1 AND visibility='public'` — only the export/anon slice is unique-by-hash (private + Minna rows may share a hash; no global UNIQUE). Example `ext_id` = `ex-<hash>` (identity-by-hash) so two cards/tiers sharing identical text reuse ONE row; Minna isn't hash-deduped (ext_id `mnn-<lesson>-<type>-<idx>`, so annotations resolve by ext_id). `ON DELETE CASCADE` from `users`.
  ...
  - `sentence_annotation` — GiNZA tokens/bunsetsu, 1:1 by `sentence_id` (**Phase 4 — NLP enrichment**). Populated by an OFFLINE batch ([../sentence-nlp/](../sentence-nlp/), `ja_ginza_electra`) → committed `data/annotations.json` → `scripts/seed-annotations.ts`; the **server only ever READS** it (no Python on the droplet). Token `start/end` are **UTF-16 offsets** into `text` (see the offset-contract dead-end). Write = `db.upsertAnnotation` (re-asserts the offset contract, throws on mismatch); read = `db.getAnnotation({extId, viewer})`, which shares the **exact same `VIEWER_VISIBLE` privacy predicate** as `getSentences` (pinned in `db/repos/annotations.test.ts`). **Grammar tags** detected by the same batch (a curated 38-point N5/N4 catalog in `sentence-nlp/patterns.py`) land in `sentence_tag(kind='grammar')` via `db.setGrammarTags` — written for `source='example'` rows only (Self-Talk keeps its hand-authored tags), reusing the study-app's `SELFTALK_GRAMMAR` ids so detected + curated grammar search through one vocabulary. **Serving (commit 3a, shipped):** `getSentences({includeAnnotations})` LEFT JOINs `sentence_annotation` INSIDE the same `VIEWER_VISIBLE`-gated query and attaches `annotation?` to each `AssembledSentence`; the route exposes it opt-in via `GET /v1/sentences?annotate=1` (off → payload unchanged). Grammar needs no serving work — it already rides `tags.grammar` on `getSentences`. The join can't widen the gate (annotations only return for rows the viewer already passes), pinned by a breach test in `db/repos/annotations.test.ts`. The study-app tap UI + grammar filter that CONSUME this are commit 3b/3c.
  ...
  - **Privacy choke-point:** every read goes through `db.getSentences({ownerType, ownerId?, viewer})` which ALWAYS ANDs `(public=1 OR created_by=:viewer)`, fail-closed (null viewer → public only), and returns **one entry PER LINK** (a reused sentence reports every owner_id/tier; `ownerId` narrows to one). Pinned breach-prevention tests live in `db/repos/sentences.test.ts` — keep them green. Writes (`createSentence`/`updateUserSentence`/`deleteUserSentence`) enforce ownership in SQL; `upsertPublicSentence` is the idempotent Self-Talk seed path; `seedExampleSentence` (reuse-by-hash via `getPublicSentenceByHash` + card-link replace) is the idempotent example seed path. `replaceUserCardExamples` (Phase 2.5) is the PRIVATE per-user analog of `seedExampleSentence`'s wholesale replace — it sets a custom card's example rows (`source='custom'`, `owner_type='card'`, `owner_id=<rank>`, `tier`, scoped to `created_by=viewer` so it never touches a public built-in) in one call, dual-written by the study app on every custom-card save so a signed-in user's custom cards render from the store like built-ins (`GET ?ownerType=card` returns the caller's own private rows through the same gate). Served by `routes/sentences.ts` (`?ownerType=selftalk|card[&ownerId=]` + `PUT /card/{rank}`); `/v1/sentences` is in the `STUDY_ROUTE` credentialed-CORS allowlist.
  - **`sentence_template` (slot-swap TEMPLATES) — Slices 1 + 2 SHIPPED.** The 独り言 Self-Talk slot-swap TEMPLATES (skeleton + slots + fillers) live in a separate `sentence_template` table — NOT `sentence` rows, since a template has no single fixed text/hash/furigana. Curator-seeded from the study-app bundle (`db.upsertPublicTemplate`, idempotent by ext_id; `seed-sentences.ts` Pass 3), served by **`GET /v1/templates[?source=]`** through `db.getTemplates`, which is the **literal mirror** of the `getSentences` `VIEWER_VISIBLE` gate (`public=1 OR created_by=:viewer`, fail-closed) — its own `TEMPLATE_VIEWER_VISIBLE` fragment + `public_template` VIEW + a pinned breach test in `db/repos/templates.test.ts`. Curator-only (no authoring write path; the private columns + gate ship so user-authored templates need no migration later). **Slice 2 — lazy materialization (SHIPPED):** the first time a signed-in user plays/records a combo, the client POSTs ONLY the picks to **`POST /v1/templates/{extId}/realize`**; the server RECONSTRUCTS the realized text/furigana/English from the stored skeleton (`lib/realize.ts`, a byte-for-byte port of the study-app's `realizeTemplate`/`plainText`/`rubyToSegments` — the runtime image carries no `study-app/`, so it's ported not imported), then `db.materializeTemplateRealization` upserts a PUBLIC `sentence` row (`source='template'`, `created_by=NULL`, idempotent by hash — mirrors `seedExampleSentence`'s reuse-by-hash) linked via `sentence_link(owner_type='template', owner_id=<template ext_id>, role=<canonical combo key>)`, copying the template's curated grammar (`db.getTemplate` reads it server-side; never client-trusted). So de-dup/export/grammar-search/TTS-pre-gen cover the combos people use. **Settled open questions** (see the design doc): #1 server-reconstructs (authoritative); #2 trigger on play **and** record (signed-in, deduped per session); #4 copy grammar; #6 `source='template'`, template-link **only** (NOT in the `ownerType=selftalk` read — never renders as a duplicate phrase card). Account-gated (it writes the PUBLIC corpus; anon keeps playing via lazy TTS). **Tap-to-lookup tokens LAG** until the next offline NLP parse picks up the now-public combo rows (no Python on prod), and that batch only rewrites `source='example'` grammar, so the copied curator grammar survives. Full design + phasing was consolidated into [../ROADMAP.html](../ROADMAP.html) (store: slot-swap templates).
...
- `recording/<userId>/<lesson>/<sanitizedItemKey>/<uuid>.webm` — a user's voice take (Minna Phase 2). **Private** object (`acl:'private'`) — served only via the gated `/v1/audio/recordings/{id}` route, never its public URL. The authoritative key lives in the `minna_recordings` row.
...
4. **Per example, in parallel batches of 4**: try IK `/download_media` proxy for audio (`Referer: immersionkit.com` spoof, treat <1KB body as miss). On miss, fall back to Google TTS (`client=gtx`, `Referer: translate.google.com` spoof, 200-char truncation). Upload to storage at the audio key. Same shape for image (no TTS fallback — image misses just leave `imageUrl: null` and clients fall back to the DDG pool). The exists→fetch→persist mechanic is **`services/mediaCache.ts:resolveMediaUrl`** (shared with the TTS + native-audio routes); the pipeline keeps only the audio/image *policy* (IK-before-TTS) + the `audioStorage`/`imageStorage` stats. Cache-hits use an `exists()` HEAD, never a byte download.
...
6. **Background: DDG fallback pool.** Fire-and-forget — `completeDdgInBackground(word)` runs after `warmWord` returns. Two-step DDG fetch (HTML page for `vqd` token, then JSON `/i.js`), up to 10 images uploaded to `ddg/<word>/N.jpg`. On completion (or failure — best-effort), re-upserts the row with the full `fallbackImages` and drops the `incomplete` flag. `ddgWarms` (a `SingleFlight`, `lib/singleFlight.ts`) dedupes overlapping background tasks per word — a duplicate is dropped, and the in-flight slot frees automatically on settle (no manual cleanup to leak).
...
| GET | `/v1/audio/tts?text=&voice=` | — | Serve a tagged synth clip via `resolveTts`. `voice` (e.g. `siri:female`) prefers its pre-generated `.m4a` then falls through to the default; omitted/`google` = default voice. `Cache-Control: public, no-cache` + a bytes-ETag (a clip is re-voiceable, so NOT immutable). |
| GET | `/v1/audio/native?src=` | Cookie | Native-audio MP3 (gated). |
| POST/GET/DELETE | `/v1/audio/recordings[?…/{id}]` | Cookie | Per-user voice takes (gated). |
| GET | `/v1/tts?text=` | — | **Legacy alias** of `/v1/audio/tts` with the default voice. Two-tier resolve (storage `tts/<sha256(text)>.{m4a,mp3}` → Google) in `resolveTts`. Kept for existing clients. 400 missing/over-200-char, 502 upstream. |
...
| GET | `/v1/progress/{app}` | Cookie | Fetch the user's progress blob (`app` ∈ {`verbs`, `custom-verbs`, `settings`, `minna`, `selftalk`, `songs`, `wanikani`, `jlpt`}). 401 if logged out. |
...
| GET | `/v1/minna/lessons/{n}` | Cookie | Curated Minna lesson JSON (`data/minna/lesson-<n>.json`), ENRICHED with GiNZA tap-to-lookup: each grammar/example/conversation sentence gets `tokens` + structured `furigana` matched from the store's gated Minna annotations by plainText hash (`enrichLessonAnnotations`; fail-soft to plain ruby). Same gate; 404 if absent. |
...
| GET | `/v1/sentences?ownerType=[&annotate=1]` | Cookie* | Unified sentence store (`ownerType=selftalk\|card`). Returns public rows + the caller's own private rows through the privacy choke-point (`db.getSentences`). `&annotate=1` adds each sentence's GiNZA token `annotation` (Phase-4 tap-to-lookup), riding the same gate. *Anon-readable (public rows only); still in `STUDY_ROUTE` because the study app's call is credentialed. `no-store`. |
...
**Error response contract** — every non-2xx response is `{ code, error, detail? }`. Switch on `code` (stable enum), never on `error` (human-readable, may change). The enum: `validation_error`, `unauthorized`, `not_found`, `conflict`, `rate_limited`, `upstream_failure`, `service_unavailable`, `internal_error`. (`conflict` is currently only used by `POST /v1/admin/warm {"scope":"all"}` when a warmAll is already in flight — returns 409; `rate_limited` is the 429 from the auth-endpoint limiter.)
...
- **DB tests use `openDb(':memory:')` + `_useDbForTesting(mem)`** in `beforeEach` for isolation, with one `*.test.ts` per repo beside its source under [src/db/repos/](src/db/repos/) (e.g. [src/db/repos/sentences.test.ts](src/db/repos/sentences.test.ts)). `openDb`/`_useDbForTesting` come from [src/db/connection.ts](src/db/connection.ts); the `_useDbForTesting` export is test-only — don't call it from app code.
...
> CLAUDE.md (module map + design-system contracts + dead-ends), CARDS, MINNA; the backlog is the repo-root ROADMAP.html. This
...
- **TTS (storage-backed, Apple-voice-preferred, voice-tagged):** the resolver lives in `resolveTts(text, voice?)` ([src/services/tts.ts](src/services/tts.ts)), shared by `GET /v1/audio/tts?voice=` (new) and `GET /v1/tts` (legacy default-voice alias). Two tiers, cheapest first: our **storage layer** → `googleTts` (`tl=ja`, 200-char cap, persisted as `.mp3` on first hit so restarts skip it) — there is intentionally NO in-process buffer cache (storage is the single source of truth, so a regenerated clip serves immediately; HTTP ETag-revalidation is the real caching tier). A **tagged voice** (e.g. `siri:female`) prefers its own pre-generated `audio/<provider>/<gender>/<hash>.m4a`, then falls through to the default `tts/<hash>.m4a` → `.mp3` → Google. `GET /v1/audio/variants?text=` lists which tagged voices exist (the `audio_variants` manifest) + an always-available `google`. `Cache-Control: public, no-cache` + a bytes-ETag (a clip is re-voiceable, so NOT immutable). The study app falls back to speechSynthesis only over `file://` or on failure.
- **Local TTS pre-generation (macOS) + Minna-audio prefetch (`scripts/`):** operator scripts feed the surfaces above, all run locally and seed our storage (point them at the prod `S3_*` env to seed prod):
  - `generate-tts.ts` — enumerates the text the study app voices (every built-in + Minna card reading via `ttsText`; every built-in/Minna leveled/lesson example sentence via `plainText`; every 独り言 Self-Talk built-in phrase AND every slot-swap template combo — the full cartesian product per template via `realizeTemplate`), renders each clip, and uploads it to `ttsKey(text,'m4a')` (the default voice). The enumeration lives in the shared `collectTtsTexts.ts` (so renders + the `audio_variants` manifest can't drift); template combos are enumerated from the BUNDLE, not the DB's materialized subset, so every combo gets a clip regardless of what's been played. Idempotent (skips clips already in storage). **`--variant <provider:gender>`** instead writes to the tagged `ttsVariantKey()` + records an `audio_variants` row, so the clip becomes a selectable voice in `/v1/audio/variants`. `--variant` only sets the output tag — `say` always uses the System Voice, so BOTH Siri genders = two passes flipping the System Voice between them (set it to a Siri MALE voice → `--variant siri:male`; flip to FEMALE → `--variant siri:female`). For prod, also point `DATABASE_FILE` at the prod sqlite so the manifest rows land where `/v1/audio/variants` reads them. Two renderers via `--engine`:
  ...
  - `prefetch-minna-audio.ts` — downloads every native-audio MP3 the curated lessons reference (vocab + conversation) into storage up front, mirroring the `/v1/audio/native` route's caching, so we never lazy-round-trip to vnjpclub.
  ...
  - `seed-sentences.ts` — seeds the built-in curator content into the store as public rows: Pass 1 = 独り言 Self-Talk phrases (`study-app/src/data/selftalk.js` → `db.upsertPublicSentence`); Pass 2 = built-in vocab example sentences (`study-app/src/data/examples.js` → `db.seedExampleSentence`, linked to cards by rank+tier, identical text reused not duplicated); Pass 3 = 独り言 Self-Talk slot-swap TEMPLATES (`study-app/src/data/selftalk-templates.js` → `db.upsertPublicTemplate`, the generator structure into `sentence_template`, NOT a `sentence` row). All idempotent. The git-tracked bundles stay the curator authoring source; this is the seed→DB step. **Must run as a deploy step** (point `DATABASE_FILE` at the prod sqlite / run on the droplet, same pattern as `generate-tts.ts`) or the prod 独り言 tab (phrases AND templates) AND the flashcard/Browse example sentences render empty.
...
- **みんなの日本語 dashboard:** account-gated `/v1/minna/*` ([src/routes/minna.ts](src/routes/minna.ts)) serves curated Minna no Nihongo lessons (`data/minna/lesson-<n>.json`, git-tracked, built from the [scripts/scrape-minna.ts](scripts/scrape-minna.ts) extractor) + proxies/caches native vnjpclub audio ([src/services/minnaAudio.ts](src/services/minnaAudio.ts), strict `/Audio/…mp3` SSRF guard, stored via the storage layer's new `get()`). Gated by `currentUser` + the optional `MINNA_OWNER_EMAILS` allowlist so the copyrighted textbook material never reaches anonymous visitors. Per-lesson notes sync under the `minna` progress key; scraped vocab activates into the study deck as tagged custom cards (client-side). **Phase 2 (record-your-voice + compare to the cached native audio) shipped:** `POST/GET/DELETE /v1/audio/recordings` store per-user voice takes as **private** storage objects (served only through the owner-scoped GET) in the `minna_recordings` table, pruned per item to the user's keep-N; `GET /v1/minna/practice` (`recordingSummary`) is a per-lesson practice-history aggregate over those rows. See [../study-app/MINNA.md](../study-app/MINNA.md) "Phase 2". Frontend contract: the みんなの日本語 dead-end in [../study-app/CLAUDE.md](../study-app/CLAUDE.md). **Full feature doc (both halves + roadmap): [../study-app/MINNA.md](../study-app/MINNA.md).**
- **Anon degradation (NOT "offline-first"):** the study app is **server-required by decision** (2026-06-14) — prefer clean server-required designs over offline-safe-but-complex ones. It still *degrades* to localStorage with no account, but that is a fallback, not a design driver, and several tabs (みんなの日本語, 歌 BYO, recordings) simply gate on an account. Signing in mirrors BOTH the progress store and the custom verbs to the server; on login the server copy wins per app key, and a brand-new account seeds the cloud from local. `save()`/`saveCustom()` → localStorage immediately + a debounced `PUT`.
