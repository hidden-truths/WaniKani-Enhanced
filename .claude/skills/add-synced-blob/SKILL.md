...
| `jlpt` | `features/jlpt/store.js` | `jpverbs_jlpt` | `null` | `mergeJlpt` (in `core/jlpt.js`) | level + examDate + targets + day record |
...
- `export const <x>Blob = createSyncedBlob({...})` with:
  ...
  - `shouldSeed: () => ...` — return true only when local state holds real user data. Why:
    ...
    (jlpt checks for a non-default level/date, any day record, or any target).
