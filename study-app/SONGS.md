...
| Concern | File |
|---|---|
| Tab — the `features/songs/` package: `index.js` (render dispatch + song-view shell + the declarative `ACTIONS` click table + navigation with the `S.nav` stale-open epoch + lifecycle) · `state.js` (the shared mutable `S` view-state) · one module per surface/mode (`library`/`add`/`edit`/`read`/`listen`/`shadow`/`mine`/`progress`). `features/songs.js` is a thin `export *` re-export so consumers' imports are unchanged. Render/navigation glue is tested in [test/songs-render.test.js](test/songs-render.test.js). | [src/features/songs/](src/features/songs/) |
...
- [x] `features/songs.js`: Library grid (filters, coverage ring, source/level badges); Add flow (paste + YouTube → analyze → review w/ flags → save, manual title/artist inputs with oEmbed fallback, graceful 503); Read viewer (furigana flip, reveal-on-tap EN, tap-a-word via overlayTokens+wireWordTaps, grammar chips → reference panel, per-line replay = synth or YouTube slice); Mine (vocab by JLPT known/added/new + per-word/bulk add under Source:歌, grammar points + counts, grammar reference + save-line-as-shadow-phrase).
