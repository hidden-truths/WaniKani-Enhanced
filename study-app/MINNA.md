...
- **More chapters** — run [../wk-enhanced-api/scripts/scrape-minna.ts](../wk-enhanced-api/scripts/scrape-minna.ts) for
  ...
  already N-lesson aware (chapter chips, `lastLesson`). **Shipped so far: L1–L50 — all of
  みんなの日本語 I AND II (2026-07-08).** (Curated from the scraped drafts; the generated Japanese
  is flagged for maintainer proofread, same status as `examples.js` — the L26–50 backfill added
  734 vocab entries, 632 with full N5→N1 leveled examples, plus reconstructed grammar/examples/
  conversation per chapter. L23 carries iTalki flags from the maintainer's lesson; the rest don't
  yet — add `italki:true` per word as those tutoring sessions happen.)
