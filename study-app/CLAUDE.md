...
- **`src/features/`** — one module per surface/concern: `chrome` (tabs/font/theme), `io`
  ...
  `data.js` ALSO owns the wave-2 MCQ bank singleton (`ensureGrammarMcq()`/`grammarMcq()`, a
  second lazy chunk); the 文法形式判断 drill itself is rendered by the JLPT tab's grammar lens
  (`features/jlpt/view.js` mcqHtml + the `mcq-*` ACTIONS) over the pure `core/grammar-mcq.js`.
...
- **The MCQ drill shuffles the CHOICES, not just the question order — and that is load-bearing.**
  ...
  answer index, so a run can never teach position instead of grammar. Don't "simplify" it back to
  rendering `q.choices` in bank order. The RNG is injected (`core/*` never calls `Math.random`), so
...
- **THREE generated grammar artifacts share ONE id vocabulary — don't merge them and don't
  hand-edit any of them.** `data/grammar.json` is the GiNZA *tagger* catalog (38 N5/N4 points,
  regenerated from `sentence-nlp/patterns.py`); `data/grammar-n3.js` is the *curriculum* (81 N3
  points + cloze content, regenerated by `tools/grammar-n3/build.mjs`); `data/grammar-n3-mcq.js`
  is the wave-2 *MCQ bank* (文法形式判断 questions per point, regenerated by
  `tools/grammar-n3/build-mcq.mjs`). The MCQ bank is a SIBLING chunk, not an `mcq` field on the
  catalog, because the catalog is on the hot path for every grammar card while the bank is only
  fetched when the drill opens; it's also sparse (10/81 points as of 2026-07-08) and grows
  independently. The catalog build cross-checks ids against the tagger catalog —
