...
// can't collide), save + rebuild once (via the shared appendCustomCards protocol). Returns how
// many were actually added (already-in-deck points skip silently — re-clicking "Add all" is
// safe). buildGrammarCard takes only (p, rank), so the helper's `today` arg is ignored: a
// grammar card deliberately carries NO `added` stamp (it's paced by grammarPerWeek, not the 語
// word quota).
export function activateGrammarPoints(pointsToAdd) {
  const have = grammarDeckIndex(state.DATA);
  return appendCustomCards(pointsToAdd, (p) => !p || !p.id || have.has(p.id), buildGrammarCard);
}
