...
import { buildWkCard } from '../../core/index.js';
...
// progress can't collide), save + rebuild once. Returns how many were actually added.
export function activateWkVocab(subjects) {
  const adds = activatableWk(subjects);
  if (!adds.length) return 0;
  const cs = loadCustom();
  for (const s of adds) { cs.seq = (cs.seq || 100) + 1; cs.verbs.push(buildWkCard(s, cs.seq)); }
  saveCustom(cs);
  rebuildData();
...
export function wkDeckCount() {
  let n = 0;
  for (const v of state.DATA) if (v.wanikani) n++;
  return n;
}
